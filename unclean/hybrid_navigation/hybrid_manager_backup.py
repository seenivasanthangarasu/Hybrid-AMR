...
class HybridManager(Node):
        ...
        if gps_now_valid != self.gps_valid:
            ...
            if self.gps_valid:
                ...
                if self.state == "GPS_MODE":
                    ...
                    self.start_slam_stack()

    # --------------------------------------------------
    # STATIC TF
    # --------------------------------------------------
    def start_static_tf(self):

        if self.static_tf_proc is not None:
            return

        self.get_logger().warn(
            "STARTING base_link -> base_footprint TF..."
        )

        self.static_tf_proc = subprocess.Popen([
            '/bin/bash',
            '-c',
            'source /opt/ros/jazzy/setup.bash && '
    ...
    # --------------------------------------------------
    # START COMPLETE SLAM STACK
    # --------------------------------------------------
    def start_slam_stack(self):

        self.get_logger().warn(
            "STARTING COMPLETE SLAM STACK..."
        )

        self.start_static_tf()
